  ...
  try {
    ...
    if (packageResult.rows.length === 0) {
      ...
      const newPackage = await client.query(`
        ...
        ("packageName", description, "totalMinutes", price, "roadmapId", "createdAt")
        VALUES 
        ('English Conversation Starter', 'Basic English conversation practice', 600, 2000000, NULL, NOW())
        RETURNING *
      `);
      packageResult.rows = newPackage.rows;
    }

    const packageId = packageResult.rows[0].id;
    console.log('Using package:', packageResult.rows[0]);
    ...
    verifyResult.rows.forEach(row => {
      console.log(`- ${row.email}: ${row.packageName} with ${row.remainingMinutes} minutes`);
    });
